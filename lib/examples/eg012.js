 ...
 */
eg012.createController = async (req, res) => {
  ...
  let accountId = req.dsAuthCodeGrant.getAccountId(),
    ...
    results = null;
  try {
    results = await eg012.worker(args);
  } catch (error) {
...
eg012.worker = async args => {
  ...
  results = await createConsoleViewP(args.accountId, { consoleViewRequest: viewRequest });
  let url = results.url;
