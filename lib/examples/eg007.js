...
eg007.createController = async (req, res) => {
  ...
  let accountId = req.dsAuthCodeGrant.getAccountId(),
    ...
    results = null;
  try {
    results = await eg007.worker(args);
  } catch (error) {
...
eg007.worker = async args => {
  ...
  results = await getEnvelopeDocumentP(args.accountId, args.envelopeDocuments.envelopeId, args.documentId, null);
