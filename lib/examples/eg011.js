 ...
 */
eg011.createController = async (req, res) => {
  ...
  let body = req.body,
    ...
    results = null;
  try {
    results = await eg011.worker(args);
  } catch (error) {
...
eg011.worker = async args => {
  ...
  results = await createSenderViewP(args.accountId, envelopeId, { returnUrlRequest: viewRequest });
