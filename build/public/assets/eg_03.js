...
let DS_EG = (function () {
    ...
    const notify_info_t = { className: "info", globalPosition: "top center" }, notify_warning_t = { className: "warn", globalPosition: "top center" };
    let library = {};
    ...
    library.json = {
        ...
        prettyPrint2: function (obj) {
            var jsonLine = /^( *)("[\w]+": )?("[^"]*"|[\w.+-]*)?([,[{])?$/gm, out = JSON.stringify(obj, null, 3)
                .replace(/&/g, "&amp;")
                .replace(/\\"/g, "&quot;")
                .replace(/</g, "&lt;")
                .replace(/>/g, "&gt;")
    ...
    function process_server_flash_msgs() {
        let flash_msg_raw = $("#server_data").attr("data-server-data"), flash_msg_json = flash_msg_raw ? JSON.parse(flash_msg_raw) : false, flash_msg_info = flash_msg_json && flash_msg_json.flash && flash_msg_json.flash.info;
    ...
    function process_json_display() {
        let json_raw = $("#server_json_data").attr("data-server-json-data"), json_raw2 = json_raw ? JSON.parse(json_raw) : false, json_raw3 = json_raw2 && json_raw2.json, json = JSON.parse(json_raw3);
        if (json) {
    ...
    function doCountdown() {
        let value = 200, timerMS = 300, el = $("#feedback h3 span"), show = () => {
